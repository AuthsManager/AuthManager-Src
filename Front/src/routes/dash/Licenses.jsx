...
export default function Licenses() {
    ...
    const { data: licenses, mutate } = useSwr(`${BASE_API}/v${API_VERSION}/users/licenses`, fetcher, { revalidateIfStale: false, revalidateOnFocus: false, revalidateOnReconnect: false });

    useEffect(() => {
        if (!user.applications.length) return navigate('/dash/apps');
        if (appId && !user.applications.find(app => app.id === appId)) setAppId('');
        if (licenses && Array.isArray(licenses)) mutate(licenses.filter(l => user.applications.find(app => app.id === l.appId)), { revalidate: false });
    }, [user, appId, licenses]);

    const createLicense = async () => {
        const response = await fetch(`${BASE_API}/v${API_VERSION}/users/licenses`, {
            method: 'POST',
            headers: {
    ...
    async function renewLicense(licenseId) {
        const response = await fetch(`${BASE_API}/v${API_VERSION}/users/licenses/${licenseId}`, {
            method: 'PATCH',
            headers: {
                'Authorization': `${location.pathname.includes('admin') ? 'Admin' : 'User'} ${localStorage.getItem('token')}`
            }
